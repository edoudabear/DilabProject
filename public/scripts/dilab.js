            ...
            loadTemplate("groups",()=>{
                ...
                fetch('/Dilab/get', {
                   ...
                   for (var i=0;i<data.length;i++) {
                       document.querySelector(".groupsWrapper").innerHTML+=newGroupElement(data[i].groupName,data[i].groupGenres,data[i].description,new Date(data[i].dateOfBirth).getFullYear(),data[i].nCollaborators,"","",data[i].groupPicture);
                    }
                });
                if (!document.querySelector(".loginButton")) {
