...
app.post("/Dilab/:action", upload.array("files"), (req,res,err) => {
    ...
    if (req.params.action=="connect") { // Path sort, THEN POST body analysis
        ...
        if (req.body.type=="mainReleases") {
            ...
            WHERE isGroupOrProject="p"AND DilabProject.name=${dilabConnection.escape(decodeURIComponent(req.body.projectName))} AND DilabMusicGroups.groupName=${dilabConnection.escape(decodeURIComponent(req.body.groupName))} ${(req.body.minTime) ? ` AND DilabChats.sendTime>=${String(parseInt(new Date (req.body.minTime).getTime()/1000))}` : ``}
