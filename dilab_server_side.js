...
app.post("/Dilab/:action", upload.array("files"), (req,res,err) => {
    if (err instanceof multer.MulterError) {
        ...
        if (req.body.type=="mainReleases") {
            ...
            SELECT releaseDate, DilabMusicGroups.groupName, projectBirthDate,name,releasePicture,duration,id,lyrics,COALESCE(nb_streams,0) AS nb_streams
            ...
            SELECT releaseDate, DilabGenres.genreName, DilabMusicGroups.groupName, projectBirthDate,name,releasePicture,duration,id,lyrics,COALESCE(nb_streams,0) AS nb_streams
            FROM DilabReleases dr
            LEFT JOIN cte ON dr.id=cte.songId
            JOIN DilabMusicGroups ON DilabMusicGroups.id=dr.groupAuthor
            ...
            SELECT releaseDate, projectBirthDate,name,releasePicture,duration,id,lyrics,COALESCE(nb_streams,0) AS nb_streams
            FROM DilabReleases dr
            LEFT JOIN cte ON dr.id=cte.songId
            JOIN DilabMusicGroups ON DilabMusicGroups.id=dr.groupAuthor
            ...
            SELECT releaseDate, DilabMusicGroups.groupName, projectBirthDate,name,releasePicture,duration,id,COALESCE(nb_streams,0) AS nb_streams
            FROM DilabReleases dr
            LEFT JOIN cte ON dr.id=cte.songId
            JOIN DilabMusicGroups ON DilabMusicGroups.id=dr.groupAuthor
